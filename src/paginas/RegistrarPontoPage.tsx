...
async function getLocation(): Promise<LocationData | null> {
  ...
  try {
    ...
    return { latitude, longitude, accuracy: Math.round(accuracy), address, city, state }
  } catch {
    return null
  }
}
...
export function RegistrarPontoPage({ user, onPointCreated }: RegistrarPontoPageProps) {
  ...
  const balance = useMemo(() => {
    ...
    const ci = toMins(entry)
    const bs = breakS ? toMins(breakS) : null
    const be = breakR ? toMins(breakR) : null
    const co = exit ? toMins(exit) : nowTime()
    const morning = bs !== null ? bs - ci : co - ci
    const afternoon = bs !== null && be !== null ? co - be : 0
    const worked = Math.max(morning + afternoon, 0)
    const expected = 480
    const saldo = worked - expected
    const extra = Math.max(saldo, 0)
    const negative = Math.max(-saldo, 0)
    return { worked, saldo, extra, negative }
  }, [todayEvents])
        ...
        <div className="flex flex-col gap-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-stretch h-full">
            ...
            <div className="bg-white dark:bg-white/[0.03] dark:backdrop-blur-sm border-gray-200/60 dark:border-white/5 rounded-xl p-5 flex flex-col h-full">
              ...
              <div className="pt-4 border-t border-default mt-auto">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                  {[
                    { label: "Usuário autenticado", ok: !!user, icon: User },
                    { label: "Matrícula válida", ok: !!user?.registrationNumber, icon: Hash },
                    { label: "Localização obtida", ok: !locationLoading && !!location, icon: MapPin },
                    { label: "Dispositivo identificado", ok: !!deviceInfo?.browser, icon: Monitor },
                  ].map((c) => {
                    const Icon = c.icon
                    return (
                      <div key={c.label} className={`flex items-center gap-3 px-3.5 py-2.5 rounded-lg ${
                        c.ok ? "bg-[var(--accent-green)]/5" : "bg-gray-50 dark:bg-white/[0.04]"
                      }`}>
                        <div className={`w-7 h-7 rounded-lg flex items-center justify-center shrink-0 ${
                          c.ok ? "bg-[var(--accent-green)]/10" : "bg-white/[0.05]"
                        }`}>
                          <Icon size={12} className={c.ok ? "text-[var(--accent-green)]" : "text-muted/30"} />
                        </div>
                        <div className="flex-1 min-w-0">
                          <span className={`text-[10px] font-medium ${c.ok ? "text-secondary" : "text-muted/40"}`}>
                            {c.label}
                          </span>
                          <div className="flex items-center gap-1 mt-0.5">
                            <CheckCircle2 size={8} className={c.ok ? "text-[var(--accent-green)]" : "text-muted/20"} />
                            <span className={`text-[8px] font-semibold uppercase tracking-wider ${
                              c.ok ? "text-[var(--accent-green)]" : "text-muted/20"
                            }`}>
                              {c.ok ? "Verificado" : "Não verificado"}
                            </span>
                          </div>
                        </div>
                      </div>
            ...
            <div className="bg-white dark:bg-white/[0.03] dark:backdrop-blur-sm border-gray-200/60 dark:border-white/5 rounded-xl p-5 flex flex-col h-full">
              ...
              {locationLoading ? (
                ...
                <div className="space-y-3 flex flex-col flex-1">
                  <div
                    className="relative rounded-lg overflow-hidden cursor-pointer group flex-1"
          ...
          <div className="bg-white dark:bg-white/[0.03] dark:backdrop-blur-sm border-gray-200/60 dark:border-white/5 rounded-xl p-5">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-2">
              {[
                ...
                { label: "IP", value: maskedIP, icon: Globe },
            ...
            {balance ? (
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div className="flex flex-col gap-0.5">
                    <span className="text-[9px] text-muted uppercase tracking-wider">Banco de horas</span>
                    <span className={`text-base font-bold font-mono ${balance.negative > 0 ? "text-[var(--accent-red)]" : "text-muted"}`}>
                      {formatTimeDisplay(-balance.negative)}
                    </span>
                  </div>
                </div>
